...
* **`config.json`**: Override default LLM models, set API keys, or tweak core engine parameters. **Multi-Proxy Support:** Define comma-separated URLs in the `"proxies"` field to auto-spawn fallback provider instances. **Vision Support:** Set `"disable_vision": true` if your chosen LLM does not support image inputs to prevent API crash loops. **Security features:** Adding a `password` field will lock down the web UI, and providing `ssl_cert` and `ssl_key` paths will automatically serve QuietForge over HTTPS.
